...
echo Python not found. Please install Python 3.12+ or 3.14+ and add it to PATH.
pause
exit /b 1
...
if %errorlevel% neq 0 (
    ...
    %PYTHON_CMD% -m pip install --no-cache-dir --upgrade pip setuptools wheel
    %PYTHON_CMD% -m pip install --no-cache-dir -r "%~dp0requirements.txt"
    if %errorlevel% neq 0 (
