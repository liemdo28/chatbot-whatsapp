...
def _get_nav_order(operator_mode: str = "standard") -> list:
    ...
    standard = ["home", "wizard_download", "toast_download", "wizard_qb", "settings", "recovery"]
    # Admin/support: full access including raw tabs and audit tools
    admin = ["home", "wizard_download", "toast_download", "wizard_qb", "download", "qb", "remove", "settings", "recovery", "audit"]
...
class App(ctk.CTk):
    def __init__(self, *, runtime_mode="gui", start_hidden=False, headless_downloads=None):
        ...
        # QB auto-start + auto-sync scheduler (non-blocking, after UI is visible)
        self._qb_startup_svc = None
        self._auto_sync_sched = None
        self._qb_activity_log_sched = None
        self.after(3000, self._start_qb_startup_service)
        self.after(8000, self._start_auto_sync_scheduler)
        self.after(12000, self._start_qb_activity_log_scheduler)
        ...
        self._init_tray()
        self._show_agent_status_badge()

    def _start_qb_startup_service(self):
        """Called 3s after startup — launches QB startup service in background."""
        if is_safe_mode():
            return
        try:
            ...
            def _on_qb_status(s):
                if hasattr(self, "home_tab"):
                    self.home_tab.update_qb_status(s.status, s.message, error=s.error)

            self._qb_startup_svc = start_qb_startup_service(
                config=load_local_config(),
                on_status=_on_qb_status,
                on_log=lambda msg: _app_logger.info("[QB] %s", msg),
            )
        except Exception as exc:
            _app_logger.warning("QB startup service failed to start: %s", exc)

    def _start_auto_sync_scheduler(self):
        """Called 8s after startup — starts the auto sync scheduler."""
        if is_safe_mode():
            return
        try:
            from services.auto_report_sync_scheduler import start_scheduler

            def _get_qb_status() -> str:
                if self._qb_startup_svc is None:
                    return "QB_DISABLED"
                return self._qb_startup_svc.get_status().status

            def _on_sched_status(s):
                if hasattr(self, "home_tab"):
                    self.home_tab.update_scheduler_status(
                        s.status, s.message,
                        last_sync=s.last_sync_at,
                        next_sync=s.next_sync_at,
                        error=s.last_error,
                    )

            self._auto_sync_sched = start_scheduler(
                config=load_local_config(),
                on_status=_on_sched_status,
                on_log=lambda msg: _app_logger.info("[AutoSync] %s", msg),
                get_qb_status=_get_qb_status,
            )
        except Exception as exc:
            _app_logger.warning("Auto sync scheduler failed to start: %s", exc)

    def _start_qb_activity_log_scheduler(self):
        """Called 12s after startup — starts QB activity log scheduler."""
        if is_safe_mode():
            return
        try:
            from services.qb_activity_log_scheduler import start_activity_log_scheduler
            ...
            def _on_alog_status(status: str, message: str):
                if hasattr(self, "home_tab"):
                    self.home_tab.update_activity_log_status(status, message)

            self._qb_activity_log_sched = start_activity_log_scheduler(
                config=load_local_config(),
                get_qb_status=_get_qb_status,
                on_status=_on_alog_status,
                on_log=lambda msg: _app_logger.info("[QBActivityLog] %s", msg),
            )
        except Exception as exc:
            _app_logger.warning("QB activity log scheduler failed to start: %s", exc)
    ...
    def _init_tray(self) -> None:
        """Initialize the system tray (Phase 4)."""
        try:
            from services.tray_service import TrayService
            # Check if background agent is running
            from services.app_single_instance import read_heartbeat
            hb = read_heartbeat()
            agent_running = hb is not None and hb.get("status") in ("AGENT_RUNNING", "AGENT_STARTING")

            self._tray_service = TrayService(
                on_open_dashboard=self._tray_open_dashboard,
                on_generate_activity_log=self._tray_generate_activity_log,
                on_generate_timeline=self._tray_generate_timeline,
                on_open_log_folder=self._tray_open_log_folder,
                on_restart_agent=self._tray_restart_agent,
                on_exit_ui=self._tray_exit_ui,
                on_stop_agent=self._tray_stop_agent,
            )
            self._tray_service.start()
        except Exception as exc:
            _app_logger.warning("System tray init failed: %s", exc)
            self._tray_service = None

    def _show_agent_status_badge(self) -> None:
        """Show whether background agent is running in the UI."""
        try:
            from services.app_single_instance import read_heartbeat
            hb = read_heartbeat()
            if hb is not None:
                status = hb.get("status", "Unknown")
                _app_logger.info("Background agent detected: %s", status)
        except Exception:
            pass

    def _tray_open_dashboard(self) -> None:
        try:
            self.after(0, lambda: self.deiconify() if self.state() == "iconic" else None)
            self.lift()
            self.focus_force()
        except Exception:
            pass

    def _tray_generate_activity_log(self) -> None:
        try:
            from services.agent_command_queue import write_command
            write_command("GENERATE_ACTIVITY_LOG_NOW", {"force": True})
        except Exception as exc:
            _app_logger.warning("Failed to trigger activity log: %s", exc)

    def _tray_generate_timeline(self) -> None:
        try:
            from services.agent_command_queue import write_command
            write_command("GENERATE_TIMELINE_NOW", {"force": True})
        except Exception as exc:
            _app_logger.warning("Failed to trigger timeline: %s", exc)

    def _tray_open_log_folder(self) -> None:
        try:
    ...
    def _tray_restart_agent(self) -> None:
        try:
            from services.agent_command_queue import write_command
            write_command("RESTART_AGENT")
        except Exception:
            pass

    def _tray_exit_ui(self) -> None:
        self.after(0, self._on_close)

    def _tray_stop_agent(self) -> None:
        try:
            from services.agent_command_queue import write_command
            write_command("STOP_AGENT")
        except Exception:
            pass

    # ── Phase 4: Close behavior ──────────────────────────────────────────

    def _on_close(self):
        # Check if background agent is running — if so, just hide the window
        try:
            from services.app_single_instance import read_heartbeat
            hb = read_heartbeat()
            agent_running = hb is not None and hb.get("status") == "AGENT_RUNNING"
        except Exception:
            agent_running = False

        if agent_running and not self.silent_mode:
            result = messagebox.askyesno(
                "Minimize instead of exit?",
                "Background agent is still running.\n"
                "Logs and QB checks will continue.\n\n"
                "Minimize to tray? (The agent stays running.)",
            )
            if result:
                self.withdraw()
                return
        ...
        if self._auto_sync_sched is not None:
            try:
                ...
                pass
        if self._qb_activity_log_sched is not None:
            try:
                self._qb_activity_log_sched.stop()
            except Exception:
                pass
    ...
    def _build_ui(self):
        ...
        all_frames = ["home", "wizard_download", "toast_download", "wizard_qb", "download", "qb", "remove", "settings", "recovery", "audit"]
        ...
        from ui.toast_download_panel import ToastDownloadPanel
        self.toast_download_panel = ToastDownloadPanel(self._tab_frames["toast_download"], status_var=self.status_var)
        self.toast_download_panel.pack(fill="both", expand=True)
