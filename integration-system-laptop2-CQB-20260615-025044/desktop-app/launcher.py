...
def main() -> int:
    ...
    # ── Handle special mode arguments ──────────────────────────────────────
    if "--background" in sys.argv:
        return _launch_background_agent()

    if "--install-startup" in sys.argv:
        return _handle_install_startup()

    if "--uninstall-startup" in sys.argv:
        return _handle_uninstall_startup()

    # --ui or no mode args → launch GUI
    # but check if background agent is already running first
    if "--ui" not in sys.argv and _is_background_agent_running():
        _log.info("Background agent is running — launching UI in observe mode.")
...
def _is_background_agent_running() -> bool:
    """Check if the background agent is already running (via heartbeat)."""
    try:
        from services.app_single_instance import read_heartbeat
        hb = read_heartbeat()
        return hb is not None and hb.get("status") in ("AGENT_RUNNING", "AGENT_STARTING")
    except Exception:
        return False


def _launch_background_agent() -> int:
    """Launch the background agent and return immediately."""
    _log.info("Launching background agent mode...")
    try:
        import background_agent
        return background_agent.main()
    except Exception as exc:
        _log.critical(f"Background agent failed: {exc}", exc_info=True)
        _show_fatal_dialog(
            "Background Agent Failed",
            f"Could not start the background agent:\n\n{exc}",
        )
        return 1


def _handle_install_startup() -> int:
    """Install the Windows startup task."""
    _log.info("Installing Windows startup task...")
    try:
        from services.windows_startup_service import install_startup
        success, message = install_startup(delay_seconds=30)
        if success:
            print(f"[OK] {message}")
            return 0
        else:
        ...
        print(f"[FAIL] install_startup failed: {exc}")
        return 1


def _handle_uninstall_startup() -> int:
    """Remove the Windows startup task."""
    _log.info("Removing Windows startup task...")
    try:
        from services.windows_startup_service import uninstall_startup
        success, message = uninstall_startup()
        if success:
            print(f"[OK] {message}")
            return 0
        else:
        ...
        print(f"[FAIL] uninstall_startup failed: {exc}")
        return 1
