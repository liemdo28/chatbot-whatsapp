...
function fail($msg){ Write-Host "[$(Get-Date -Format 'HH:mm:ss')] [X] $msg" -ForegroundColor Red }
...
# COLLECT mode creates: dist/ToastPOSManager/ToastPOSManager/_internal/...
$bundleInternal = Join-Path $bundleDir "ToastPOSManager"
...
# ---- 7. Collect playwright browser ----
# Chromium is already in the COLLECT bundle (_internal/playwright_browser/)
# Only copy if the source playwright directory is available
if ($chromiumFound) {
    $srcBrowserDir = Split-Path (Find-PlaywrightBrowser) -Parent
    if ($srcBrowserDir -and (Test-Path $srcBrowserDir)) {
        $destBrowser = Join-Path $bundleInternal "playwright_browser"
        if (-not (Test-Path $destBrowser)) {
            New-Item -ItemType Directory -Force -Path $destBrowser | Out-Null
            Copy-Item -Path $srcBrowserDir -Destination $destBrowser -Recurse -Force
            ok "Chromium synced from: $srcBrowserDir"
        } else {
            ok "Chromium already bundled"
        }
    }
} else {
    warn "Skipping Chromium -- playwright not found"
}

# ---- 8. Artifact validation ----
log "[8/11] Validating build artifacts..."
$exePath = Join-Path $bundleInternal "ToastPOSManager.exe"
Assert-Artifact (Test-Path $exePath) "ToastPOSManager.exe exists"
Assert-Artifact (Test-Path "$bundleInternal\_internal\version.json") "version.json in bundle"
Assert-Artifact (Test-Path "$bundleInternal\_internal\app.pyc") "app.py bundled"
Assert-Artifact (Test-Path "$bundleInternal\_internal\bootstrap_runtime.pyc") "bootstrap_runtime bundled"
if ($chromiumFound) {
    Assert-Artifact (Test-Path "$bundleInternal\playwright_browser") "playwright_browser/ folder present"
    Assert-Artifact ((Get-ChildItem "$bundleInternal\playwright_browser" -Recurse -File | Measure-Object).Count -gt 0) "Chromium payload non-empty"
}
Assert-Artifact (Test-Path "$bundleDir\PORTABLE_MODE.txt") "PORTABLE_MODE.txt not in bundle root"
ok "Artifact validation complete"

# ---- 9. Portable zip + checksums ----
# Source is the nested folder (dist/ToastPOSManager/ToastPOSManager)
New-PortableZip $bundleInternal $zipPath
...
if ($iscc -and (Test-Path "$PSScriptRoot\installer\ToastPOSManager.iss")) {
    ...
    log "[11/11] Inno Setup not found -- skipping installer build"
}
