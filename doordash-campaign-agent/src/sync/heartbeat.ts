/**
 * Heartbeat Scheduler
 * Pushes laptop1 status to CEO PC's doordash-campaigns app (:3000)
 * every 5 minutes so CEO can monitor remote machines dashboard.
 *
 * CEO PC endpoints:
 *   - MI-CORE :4001 — source of truth (policies, playbooks)
 *   - doordash-campaigns :3000 — receives heartbeats, shows Remote Machines dashboard
 */
...
function getMachineId(): string {
    return getSetting('machine_id', 'laptop-01');
}
...
function getCeoAppUrl(): string {
    ...
    // Derive CEO app URL from MI_CORE_URL: same host, port 3000
    const base = getSetting('mi_core_url', process.env['MI_CORE_URL'] ?? '');
    if (!base) return '';
    // Replace port with 3000 (doordash-campaigns on CEO PC)
...
function collectPayload(): Record<string, any> {
    ...
    try {
        ...
        if (loop) {
            ...
            loopLastRunAt = loop.completed_at ?? loop.started_at;
            loopLastResult = loop.status;
        }
    ...
    try {
        doordashConnected = (db.prepare(`SELECT value FROM settings WHERE key='dd_session_status'`).get() as any)?.value === 'connected';
    } catch { /* ignore */ }

    let snapshotCountToday = 0;
    try {
        snapshotCountToday = (db.prepare(`SELECT COUNT(*) AS n FROM campaign_snapshots WHERE DATE(created_at)=DATE('now')`).get() as any)?.n ?? 0;
    } catch { /* table may not exist yet */ }
    ...
    try {
        ...
        if (le) {
            lastExecutionAt = le.executed_at;
            lastExecutionResult = `${le.action}: ${le.result}`.slice(0, 100);
        }
    } catch { /* table may not exist yet */ }

    // MI-CORE sync status
    let miCoreConnected = false, lastMiCoreSync = null as string | null;
    try {
        const syncRow = db.prepare(`SELECT sync_status, synced_at FROM mi_core_sync ORDER BY created_at DESC LIMIT 1`).get() as any;
        if (syncRow) {
            miCoreConnected = syncRow.sync_status === 'success';
            lastMiCoreSync = syncRow.synced_at;
        }
    } catch { /* ignore */ }
    ...
    try {
        ...
        for (const s of stores) {
            const last = db.prepare(
                `SELECT week_start, roas FROM campaign_snapshots WHERE store_id=? ORDER BY created_at DESC LIMIT 1`
            ).get(s.id) as any;
            storeStatuses[s.id] = {
                name: s.name,
                last_snapshot_week: last?.week_start ?? null,
                last_roas: last?.roas ?? null,
            };
        }
    } catch { /* ignore */ }
    ...
    return {
        ...
        mi_core_connected: miCoreConnected,
        last_mi_core_sync: lastMiCoreSync,
...
async function sendHeartbeat(): Promise<void> {
    ...
    if (!url) {
        console.warn('[Heartbeat] No CEO app URL configured. Skipping heartbeat.');
        return;
    }
    ...
    try {
        ...
        if (!res.ok) {
            console.warn(`[Heartbeat] Rejected by CEO app: HTTP ${res.status}`);
        } else {
            console.log(`[Heartbeat] Pushed status to CEO app at ${url}`);
        }
    } catch (e: any) {
...
export function startHeartbeatScheduler(): void {
    ...
    // First heartbeat after 30 seconds (let server stabilize)
    setTimeout(() => {
        sendHeartbeat();
