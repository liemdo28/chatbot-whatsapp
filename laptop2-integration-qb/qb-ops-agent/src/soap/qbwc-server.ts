...
import { storeQbData, getLastSyncStatus } from './qb-data-store';
...
function requireQbwcPassword(): string {
  ...
  if (!value) {
    throw new Error('Missing required QBWC_PASSWORD configuration. Populate qb-ops-agent/.env before startup.');
  }
  return value;
}
